...
async function run() {
  ...
  if (command === "analytics") {
    ...
    const [packs, auditEvents] = await Promise.all([
      store.listPacks(parsed.values.tenant),
      store.listAuditEvents({ tenantId: parsed.values.tenant })
    ]);
    process.stdout.write(
  ...
  if (command === "sync-issues") {
    ...
    const targets =
      (parsed.values.target?.filter(
        (entry): entry is "github" | "jira" | "linear" => entry === "github" || entry === "jira" || entry === "linear"
      ) ?? ["github", "jira"]);
  ...
  if (command === "export-issue") {
    ...
    const issuePath = await store.exportIssueDraft(
      ...
      parsed.values.target === "jira" || parsed.values.target === "linear" ? parsed.values.target : "github"
    );
    process.stdout.write(
  ...
  if (command === "export-replay") {
    ...
    const tenantId = parsed.values.tenant ?? "default";
    const record = await store.getPack(ticketId, tenantId);
    if (!record) {
      throw new Error(`Repro pack not found: ${ticketId}`);
    }
  ...
  if (command !== "process" && command !== "repro") {
